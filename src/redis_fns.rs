...
pub async fn get_oauth_token_in_redis(oauth_token: &str) -> Result<bool, RedisError> {
    // Get a connection from the REDIS_POOL
    let mut conn = get_redis_cnxn().await?;
...
pub async fn set_oauth_token_in_redis(oauth_token: &str) -> Result<(), RedisError> {
    // Get a connection from the REDIS_POOL
    let mut conn = get_redis_cnxn().await?;
...
pub async fn get_remaining_allowance(account_id: &AccountId) -> Result<u64, RedisError> {
